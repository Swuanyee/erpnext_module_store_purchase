...
frappe.ui.form.on('Store Purchase', {
	...
	validate: function(frm) {
		...
		$.each(frm.doc.store_purchase_detail || [], function(i, store_purchase_detail){
			frm.set_value('total_amount', frm.doc.total_amount + store_purchase_detail.paid_amount);
		});
		/* Update the total advance amount */
		frm.set_value('total_advance_amount', 0);
		$.each(frm.doc.advances || [], function(i, advance){
			frm.set_value('total_advance_amount', frm.doc.total_advance_amount + advance.allocated_amount);
		});
		/* Update the grand total amount */
		frm.set_value('grand_total', 0);
		$.each(frm.doc.store_purchase_detail || [], function(i, store_purchase_detail){
			frm.set_value('grand_total', frm.doc.grand_total + store_purchase_detail.amount);
		});
	},
	on_load: function(frm) {
		...
		/* Update the total advance amount */
		frm.set_value('total_advance_amount', 0);
		$.each(frm.doc.advances || [], function(i, advances){
			frm.set_value('total_advance_amount', frm.doc.total_advance_amount + advances.allocated_amount);
		});
		/* Update the grand total amount */
		frm.set_value('grand_total', 0);
		$.each(frm.doc.store_purchase_detail || [], function(i, store_purchase_detail){
	...
	store_purchase_detail_remove: function(frm) {
		/* Update the total amount */
		frm.set_value('total_amount', 0);
		$.each(frm.doc.store_purchase_detail || [], function(i, store_purchase_detail){
			frm.set_value('total_amount', frm.doc.total_amount + store_purchase_detail.paid_amount);
	...
	advances_remove: function(frm) {
		...
		frm.set_value('grand_total', 0);
		$.each(frm.doc.store_purchase_detail || [], function(i, store_purchase_detail){
			frm.set_value('grand_total', frm.doc.grand_total + store_purchase_detail.paid_amount);
		});
	}
});
